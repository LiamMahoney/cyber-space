...
//TODO: update score on screen as soon as round is over
{
    ...
    if (msie !== -1 || triedent !== -1) {
        document.getElementById("NO-IE").innerHTML =
            "<div class='no-ie'><img src='/pics/title.png'><div class='no-ie-text'>Internet Explorer is not supported for this game. Please use Google Chrome or Microsoft Edge. Sorry for any inconvinience. </div></div>";
        ...
        PIXI.loader
            .add([
                ...
                "pics/ammo.png",
                "pics/enemyAmmo.png",
                "pics/menuBackground.png",
                "pics/controls.png",
                "pics/startButton.png",
                "pics/nextButton.png",
                "pics/life.png",
                "pics/1Phishing.png",
                "pics/2MaliciousLink.png",
                "pics/3MaliciousAttachment.png",
                "pics/4Malware.png",
                "pics/5Ransomware.png",
                "pics/6Vishing.png",
                "pics/7InsiderRisk.png",
                "pics/8Cybercriminal.png"
            ])
            .load(initialSetup);
        ...
        function initializeEnemies() {
            ...
            for (i = 0; i < 8; i++) {
                var tempEnemy = new PIXI.Sprite(
                    PIXI.loader.resources["pics/" + enemy.picture].texture
                );
                if (!enemyList[i - 1]) {
        ...
        function initializeThreatIdentifier() {
            var threatIdentifier = new PIXI.Text(
                enemyTypeList[roundNumber - 1].name,
                textStyle
            );
        ...
        function initializeLifeCounter() {
            for (i = 0; i < lives; i++) {
                //purposefully makes i available after for loop
                var tempLifePic = new PIXI.Sprite(
                    PIXI.loader.resources["pics/life.png"].texture
                );
        ...
        function updateEnemy(enemyVY) {
            ...
            for (var i = 0; i < enemyList.length; i++) {
                ...
                if (Math.floor(Math.random() * enemyFireRate + 1) === 4) {
                    var tempEnemyAmmo = new PIXI.Sprite(
                        PIXI.loader.resources["pics/enemyAmmo.png"].texture
                    );
                    tempEnemyAmmo.position.set(
                        enemyList[i].x + 8,
                        enemyList[i].y + 20
                    );
                    if (enemyVX > 0) {
                ...
                if (enemyList[i].y > 500) {
                    ...
                    roundScore = 0;
                    if (lives === 0) {
                        //due to limitations to scoreboard
                        score -= 40;
                    }
        ...
        function titleMenu() {
            var title = new PIXI.Sprite(
                PIXI.loader.resources["pics/title.png"].texture
            );
            ...
            var startButton = new PIXI.Sprite(
                PIXI.loader.resources["pics/startButton.png"].texture
            );
            startButton.y = 500;
        ...
        function objectiveMenu() {
            var container = new PIXI.Sprite(
                PIXI.loader.resources["pics/menuBackground.png"].texture
            );
            ...
            var objective1 = new PIXI.Text(
                "As a team member, you are our organization's first line of defense in a malicious attack! Fly through cyber space and block malicious attackers from stealing our company data.",
                objectiveStyle
            );
            var objective2 = new PIXI.Text(
                "Top score winner will receive a Nintendo Entertainment System: NES Classic Edition!",
                objectiveStyle
            );
            objective1.y =
                (container.height - (objective1.height + objective2.height)) /
                    2 -
                50;
            ...
            var next = new PIXI.Sprite(
                PIXI.loader.resources["pics/nextButton.png"].texture
            );
        ...
        function controlsMenu() {
            ...
            instructionHeader.x =
                (container.width - instructionHeader.width) / 2;
            ...
            var controls = new PIXI.Sprite(
                PIXI.loader.resources["pics/controls.png"].texture
            );
            controls.height = 100;
            controls.width = 599;
            ...
            var scoringRules = new PIXI.Text(
                "Each completed round gives you 20 points. Each enemy that you hit gives you 25 points. Each failed round subtracts all of the points you gained from that round and an additional 40 points. Each shot that you fire subtracts 1 point.",
                textStyle
            );
            scoringRules.x = 150;
        ...
        function quizMenu() {
            ...
            var question = new PIXI.Text(
                enemyTypeList[roundNumber - 2].quiz.question,
                objectiveStyle
            );
            ...
            var header = new PIXI.Text(
                "Click on the answer you think is correct. If your answer is correct you will be awarded an additional 60 points.",
                quizInstructionsStyle
            );
            ...
            for (
                var i = 0;
                i < enemyTypeList[roundNumber - 2].quiz.answers.length;
                i++
            ) {
                var tempAnswer = new PIXI.Text(
                    enemyTypeList[roundNumber - 2].quiz.answers[i],
                    textStyle
                );
        ...
        function checkAnswer(obj) {
            ...
            var userAnswer = new PIXI.Text(
                "You answered: " + obj.currentTarget._text,
                textStyle
            );
            ...
            if (
                obj.currentTarget._text ===
                enemyTypeList[roundNumber - 2].quiz.correctAnswer
            ) {
                score += 60;
                app.stage.children[1].text = "SCORE: " + (score + roundScore);
                var userCorrect = new PIXI.Text(
                    "This is correct! You have been awarded and additional 60 points!",
                    textStyle
                );
            ...
            var afterQ = new PIXI.Text(
                enemyTypeList[roundNumber - 2].quiz.afterQuiz,
                quizInstructionsStyle
            );
            if (correctAns) {
            ...
            var nextButton = new PIXI.Sprite(
                PIXI.loader.resources["pics/nextButton.png"].texture
            );
        ...
        function scoreMenu() {
            ...
            container.innerHTML +=
                "<div class='text header'>Score: " + score + "</div>";

            var formContainer = document.createElement("div");
            formContainer.classList.add("form");
            formContainer.innerHTML +=
                "<div class='status-container hide'></div>";
            formContainer.innerHTML +=
                "<div class='text label'>Name:</div><input type='text' id='name' class='score-input form-input'>";

            container.appendChild(formContainer);

            container.innerHTML +=
                "<div class='button button-text' onclick='submitScore()'>SUBMIT SCORE</div>";
        ...
        function displayScores(scores) {
            ...
            for (let i = 0; i < scores.length; i++) {
                let row = `<div class="score-row"><div class="score-item">${i +
                    1}</div><div class="score-item">${
                    scores[i].score
                }</div><div class="score-item">${scores[i].name}</div></div>`;
                scoreboard += row;
            }
            scoreboard += `</div>`;
            scoreMenu.innerHTML = scoreboard;
            scoreMenu.innerHTML +=
                "<div class='button button-text' onclick='playAgain()'>PLAY AGAIN</div>";
        }
        ...
        function getScoreboard() {
            ...
            xhttp.open("GET", "https://cyber-space.liammahoney.me/score", true);
            xhttp.send(null);
        }
        ...
        function addSuccessNotification() {
            ...
            container.innerHTML +=
                "<div class='success-notification-container'><div class='notification-text'>Score Successfully Submitted!</div></div>";
            container.classList.remove("hide");
        }
        function addErrorNotification() {
            var container = document.querySelector(".status-container");
            container.innerHTML +=
                "<div class='error-notification-container'><div class='notification-text'>There has been an error. \n This issue and your score have been reported.</div></div>";
            container.classList.remove("hide");
        }
        function playAgain() {
            location.reload();
        ...
        function buildErrorMessage(error) {
            ...
            for (var x = 0; x < userInputList.length; x++) {
                var temp =
                    "**" +
                    userInputList[x].id.toUpperCase() +
                    ":** " +
                    userInputList[x].value +
                    "\n";
                message += temp;
            }
        ...
        function keyboardListener() {
            ...
            spacebar.press = function() {
                ...
                var newAmmo = new PIXI.Sprite(
                    PIXI.loader.resources["pics/ammo.png"].texture
                );
